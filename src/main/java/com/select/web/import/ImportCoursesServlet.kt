...
@WebServlet(name = "ImportCoursesServlet", value = ["/ImportCoursesServlet"])
class ImportCoursesServlet : HttpServlet() {
            ...
            if (formItems != null && formItems.size > 0) {
                ...
                for (item in formItems) {
                    ...
                    if (!item.isFormField()) {
                        ...
                        var courses = csvDataImport.importCourses(filePath)

                        for (course in courses){
                            println(course.toString())
                            println("save: ${courseService.saveCourse(course)}")
                        }
